...
class LeggedRobot(BaseTask):
    ...
    def _init_buffers(self):
        ...
        self.torques = torch.zeros(self.num_envs, self.num_actions, dtype=torch.float, device=self.device,
                                   requires_grad=False)
        self.action_avg = torch.zeros(self.num_envs, self.num_actions, dtype=torch.float, device=self.device,
                                      requires_grad=False)  # doghome
        ...
        self.ref_dof_pos = torch.zeros(self.num_envs, self.num_dof, dtype=torch.float, device=self.device,
                                       requires_grad=False)
        ...
        self.trot_pattern1 = torch.tensor([1., 0, 0, 1.], dtype=torch.float, device=self.device,
                                          requires_grad=False).view(1, -1)
        self.trot_pattern2 = torch.tensor([0., 1., 1., 0.], dtype=torch.float, device=self.device,
                                          requires_grad=False).view(1, -1)
    ...
    def _create_envs(self):
        ...
        if self.cfg.domain_rand.randomize_lag_timesteps:
            ...
            self.randomized_lag = [random.randint(0, self.cfg.domain_rand.lag_timesteps - 1) for i in
                                   range(self.num_envs)]
            self.randomized_lag_tensor = torch.FloatTensor(self.randomized_lag).view(-1, 1) / (
    ...
    def compute_observations(self):  # guance
        ...
        if self.cfg.terrain.measure_heights:
            ...
            self.obs_buf = torch.cat([obs_buf, heights, priv_latent, self.obs_history_buf.view(self.num_envs, -1)],
                                     dim=-1)
        else:
    ...
    def _reset_root_states(self, env_ids):
        ...
        if self.custom_origins:
            ...
            self.root_states[env_ids, :3] += self.env_origins[env_ids]
        # base velocities
        self.root_states[env_ids, 7:13] = torch_rand_float(-0.35, 0.35, (len(env_ids), 6),
    ...
    def _reward_orientation(self):
        # 鼓励底盘保持水平稳定，利用的是四元数的投影重力向量
        return torch.sum(torch.square(self.projected_gravity[:, :2]), dim=1)

    def _reward_orientation_eular(self):
        """
        综合欧拉角和重力投影两种信息，对机器人底盘姿态偏离水平的程度进行奖励。它希望底盘绕 x、y 轴旋转角度尽可能接近 0，同时重力投影在底盘平面上的分量要最小化。
    ...
    def _reward_default_joint_pos(self):
        """
        本函数用于鼓励机器人关节位置贴近预设的“默认”姿态，尤其关注绕机器人前后、左右（roll）方向的偏差。通过对 roll 关节偏差的指数惩罚与整体关节偏差项的线性惩罚相结合，实现既严格约束关键姿态，又允许次要关节有小幅自由度。
    ...
    def _reward_joint_ref_pos(self):  # 期望关节角度轨迹
        """
        本函数基于当前关节位置与参考轨迹位置之间的差异计算奖励，旨在鼓励机器人驱动自由度紧跟预设的关节轨迹，仅在收到有效运动命令时才发放奖励。
    ...
    def _reward_vel_mismatch_exp(self):
        """
        本函数通过对机器人线速度垂直分量和底盘横滚/俯仰角速度的偏差进行指数衰减奖励，鼓励机器人保持稳定、平滑的运动状态，避免上下抖动和大幅角速度振荡。
    ...
    def _reward_track_vel_hard(self):
        """
        本函数旨在精确跟踪给定的线速度和角速度命令，通过对偏差进行不同形式的惩罚和奖励，鼓励机器人在 xy 平面上按目标速度前进，并以目标角速度转向。
    ...
    def _reward_low_speed(self):
        """
        该函数根据机器人实际前向速度与命令速度的对比，对运动状态进行分段奖励或惩罚，确保机器人既不跑得过慢，也不会超速，并且行进方向要与指令方向一致。
    ...
    def _reward_tracking_lin_vel(self):  # 在误差较小时奖励接近于1，而在误差较大时奖励接近于0
        ...
        return 0.5 + 0.5 * torch.exp(
            -error / self.cfg.rewards.tracking_sigma)  # tracking_sigma == 0.5 标准差参数，控制着指数函数的衰减速度
    ...
    def _reward_feet_air_time(self):
        ...
        rew_airTime *= torch.norm(self.commands[:, :2],
                                  dim=1) > self.cfg.rewards.command_dead  # no reward for zero command
    ...
    def _reward_no_fly(self):
        ...
        return double_contact * (torch.norm(self.commands[:, :2],
                                            dim=1) > self.cfg.rewards.command_dead)  # 1.*((self.time_to_stand_still <= self.static_delay) & single_contact)
    ...
    def _reward_feet_clearance_inter(self):
        ...
        height_error = torch.square(self.feet_height - abs(self.cfg.rewards.clearance_height_target)).view(
            self.num_envs, -1)
        foot_leteral_vel = torch.sqrt(torch.sum(torch.square(footvel_in_body_frame[:, :, :2]), dim=2)).view(
            self.num_envs, -1)
        return torch.sum(height_error * foot_leteral_vel, dim=1)
    ...
    def _reward_stumble(self):
        # Penalize feet hitting vertical surfaces足端碰撞检查
    ...
    def _reward_feet_contact_forces(self):
        ...
        return torch.sum((torch.norm(self.contact_forces[:, self.feet_indices, :],
                                     dim=-1) - self.cfg.rewards.max_contact_force).clip(0, 100), dim=1)
    ...
    def _reward_feet_distance(self):
        ...
        foot_dist1 = torch.norm(foot_pos[:, 0, :] - foot_pos[:, 3, :],
                                dim=1)  # *torch.logical_or(contact_filt[0], contact_filt[3])
        foot_dist2 = torch.norm(foot_pos[:, 1, :] - foot_pos[:, 2, :],
                                dim=1)  # *torch.logical_or(contact_filt[1], contact_filt[2])
        # print(foot_dist1[0])
        ...
        return (torch.exp(-torch.abs(torch.min(d_min1, d_min2)) * 100) + torch.exp(
            -torch.abs(torch.max(d_max1, d_max2)) * 100)) / 2
    ...
    def _reward_hip_pos(self):
        return (0.8 - torch.abs(self.commands[:, 1])) * torch.sum(
            torch.square(self.dof_pos[:, [0, 3, 6, 9]] - self.default_dof_pos[:, [0, 3, 6, 9]]), dim=1)
    ...
    def _cost_torque_limit(self):
        ...
        return 1. * (torch.sum(1. * (torch.abs(self.torques) > self.torque_limits * self.cfg.rewards.soft_torque_limit),
                               dim=1) > 0.0)
    ...
    def _cost_dof_vel_limits(self):
        return 1. * (torch.sum(
            1. * (torch.abs(self.dof_vel) > self.dof_vel_limits * self.cfg.rewards.soft_dof_vel_limit), dim=1) > 0.0)

    def _cost_vel_smoothness(self):
        return torch.mean(
            torch.max(torch.zeros_like(self.dof_vel), torch.abs(self.dof_vel) - (self.dof_vel_limits / 2.)), dim=1)
    ...
    def _cost_collision(self):
        return 1. * (torch.sum(
            1. * (torch.norm(self.contact_forces[:, self.penalised_contact_indices, :], dim=-1) > 0.1), dim=1) > 0.0)

    def _cost_feet_contact_forces(self):
        # penalize high contact forces
        return 1. * (torch.sum(1. * (torch.norm(self.contact_forces[:, self.feet_indices, :],
                                                dim=-1) > self.cfg.rewards.max_contact_force), dim=1) > 0.0)
    ...
    def _cost_foot_regular(self):  # 摆动约束
        ...
        height_error = torch.clamp(
            torch.exp(footpos_in_body_frame[:, :, 2] / (0.025 * self.cfg.rewards.base_height_target)).view(
                self.num_envs, -1), 0, 1)
        foot_leteral_vel = torch.sqrt(torch.sum(torch.square(footvel_in_body_frame[:, :, :2]), dim=2)).view(
            self.num_envs, -1)
        return torch.sum(height_error * foot_leteral_vel, dim=1)
