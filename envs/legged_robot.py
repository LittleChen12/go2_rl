...
class LeggedRobot(BaseTask):
    ...
    def _reward_default_joint_pos(self):
        ...
        torch.exp是指数函数，roll 越接近 0，torch.exp(-roll * 10) 接近 1。
        严格关注髋关节的电机 允许膝关节等非关键关节小幅运动（如行走时的必要摆动），而不会大幅降低奖励
        """
        joint_diff = self.dof_pos - self.default_dof_pos # 所有电机
        roll_FL = joint_diff[:, 0] # 髋关节的电机
        roll_FR = joint_diff[:, 3] # 髋关节的电机
        roll_RL = joint_diff[:, 6] # 髋关节的电机
        roll_RR = joint_diff[:, 9] # 髋关节的电机
    ...
    def _reward_joint_ref_pos(self):# 期望关节角度轨迹
        ...
        r *= (torch.norm(self.commands[:, :2], dim=1) > 0.3)
        return r
    ...
    def _reward_foot_clearance(self):
        ...
        惩罚的是高度不够，和x，y的平面的速度，如果运动，速度肯定有，这时候只有减少height_error，相反同理
        """
    ...
    def _reward_feet_contact_number(self):
        ...
        目标是确保机器人在运动时保持足够的脚部接触地面数量以实现稳定步态
        机器人运动时候，如果有3个脚同时接触地面，给出奖励
    ...
    def _cost_hip_pos(self):
        ...
        return (1.5-torch.abs(self.commands[:,1])) *  torch.sum(torch.square(self.dof_pos[:, [0, 3, 6, 9]] - self.default_dof_pos[:, [0, 3, 6, 9]]), dim=1)
